...
two = servo(a2, 'D10','MinPulseDuration', 1*10^-3, 'MaxPulseDuration', 2*10^-3);

%First kick
for pos = 0:0.01:1  
    writePosition(two, pos);  %Fully extend to 100%  
end
...
for i = 1:3 
    for pos = 1:-0.01:0.6
        writePosition(two,pos);   %Retract to 60% of maximum length
    end
    pause(2);
    for pos = 0.6:0.01:1
        writePosition(two,pos);   %Extend again to 100% max length
    end
    pause(2);
end

for pos = 1:-0.01:0
    writePosition(two,pos);   %Brings back to min length
end
